...
function fetchProduct(url, search, setProduct) {
    url = url + search
    async function fetchContents() {
        let res = await fetch(url);
        let json = await res.json();
        let arrayProducts = Object.values(json)
        setProduct(arrayProducts[0])
        console.log(arrayProducts[0])
    }
    fetchContents();
}

export default function Search({ searchUrl, setProduct }) {
